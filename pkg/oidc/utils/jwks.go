...
func NewRemoteKeySet(client *http.Client, jwksURL string) oidc.KeySet {
	return newRemoteKeySet(client, jwksURL, time.Now)
}

func newRemoteKeySet(client *http.Client, jwksURL string, now func() time.Time) *remoteKeySet {
	if now == nil {
		now = time.Now
	}
	return &remoteKeySet{httpClient: client, jwksURL: jwksURL, now: now}
}
