...
func TestVerifier_VerifyIDToken(t *testing.T) {
	...
	token := &oidc.IDTokenClaims{Audiences: []string{"client_id"}, AuthTime: now.Add(-3 * time.Minute), Expiration: now.Add(1 * time.Minute), IssuedAt: now.Add(-1 * time.Minute), Issuer: "https://issuer"}
	tokenWithSig := func() *oidc.IDTokenClaims { t := *token; t.Signature = jose.SignatureAlgorithm("RS256"); return &t }()

	tests := []struct {
		name     string
		verifier oidc.Verifier
		token    *oidc.IDTokenClaims
		want     *oidc.IDTokenClaims
		wantErr  bool
	}{
